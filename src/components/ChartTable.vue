  ...
  <div>
    <div class="chart--wrapper" inline>
      <v-row no-gutters class="chart--row">
        ...
        <v-col class="chart--head chart--head-right" v-if="chart.HasStun" cols="1"><b>Stun</b></v-col
      ...
      <div v-for="(item, index) in chart.Roll">
        <v-row align="stretch" no-gutters class="chart--row">
          ...
          <v-col class="chart--cols chart--cols-right" v-if="chart.HasStun" cols="1">{{ chart.Stun(index) }} </v-col>
          <v-col class="chart--cols chart--cols-right" v-if="chart.HasDamage" cols="2">{{ chart.Damage(index) }} </v-col>
          <v-col class="chart--cols chart--cols-right" cols="8"><display-tooltip-text :string="chart.Status(index)" /> </v-col>
        </v-row>
      </div>
    </div>
  </div>
