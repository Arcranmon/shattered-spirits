<template>
  <div class="chart--wrapper" inline>
    <v-row no-gutters class="chart--row">
      ...
      ><v-col class="chart--head chart--head-right" cols="6"><b>Status</b></v-col>
    </v-row>
    <div v-for="(item, index) in chart.Roll">
      <v-row align="stretch" no-gutters class="chart--row">
        <v-col class="chart--cols justify-center align-center" v-bind:class="item" cols="2">{{ index + 2 }} </v-col>
        <v-col class="chart--cols chart--cols-right" v-if="(chart.HasDamage)" cols="2">{{ chart.Damage(index) }} </v-col>
        <v-col class="chart--cols chart--cols-right" cols="6"><display-tooltip-text :string="chart.Status(index)" /> </v-col>
      </v-row>
    </div>
  </div>
</template>
