...
import { Discipline, Technique, Stance, Armor, Weapon, Npc } from "@/class";
import { Dictionary } from "vue-router/types/router";

let spiritTypes: Array<string> = [
  "Earth",
  "Flame",
  "Metal",
  "Water",
...
let allGlossaryItems: Array<Array<IGlossaryData>> = [
  Glossary,
  Statuses,
  Conditions,
  ElementalConditions,
...
export class DatabaseJsonStore extends VuexModule {
  ...
  get getPlayerArmors(): any {
    return () => {
      return Armors.filter((x) => x.category.trim() !== "NPC").map((x) =>
        Armor.Deserialize(<IArmorData>x)
      );
    };
  }
  ...
  get getWeaponsForCharCreation(): any {
    return () => {
      return Weapons.filter(
        (x) =>
          x.category.trim() !== "Unarmed" &&
          x.category.trim() !== "Improvised" &&
          x.category.trim() !== "NPC"
      ).map((x) => Weapon.Deserialize(<IWeaponData>x));
    };
  }
  ...
  get getStancesByDiscipline(): any {
    return (discipline: string) => {
      return Stances.filter(
        (x) => x.discipline.trim() === discipline.trim()
      ).map((x) => Stance.Deserialize(<IStanceData>x));
    };
  }
  ...
  get getTechniquesByDiscipline(): any {
    return (discipline: string) => {
      return Techniques.filter(
        (x) => x.discipline.trim() === discipline.trim()
      ).map((x) => Technique.Deserialize(<ITechData>x));
    };
  }
  ...
  get getGlossaryItemFromJson(): any {
    return (source: Array<IGlossaryData>, inword: string) => {
      ...
      if (keyword.hasOwnProperty("see"))
        return this.getGlossaryItemFromJson(source, keyword.see);
  ...
  get getSpiritBeasts(): any {
    return () => {
      return NPCs.filter((x) => x.npc_type.trim() === "Spirit Beast").map((x) =>
        Npc.Deserialize(<INpcData>x)
      );
    };
  }
}
